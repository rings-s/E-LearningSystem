...
<div class={classNames('form-group space-y-2', className)}>
	...
	<div class={containerClasses}>
		...
		{#if showPasswordToggle && type === 'password'}
			<button
				...
				class="absolute inset-y-0 right-0 z-10 flex items-center pr-4 text-gray-400 transition-colors duration-200 hover:text-gray-600 dark:hover:text-gray-300"
				tabindex="-1"
			>
				{#if showPassword}
					<svg
	...
	{#if hint && !shouldShowError}
		<p class="pl-1 text-xs text-gray-500 dark:text-gray-400" transition:slide={{ duration: 200 }}>
			{hint}
		</p>
	{/if}
	...
	{#if shouldShowError}
		<p
			class="flex items-center gap-1.5 pl-1 text-xs text-red-600 dark:text-red-400"
