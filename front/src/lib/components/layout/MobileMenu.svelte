...
{#if mobileMenuOpen}
	<div class="lg:hidden">
		...
		<!-- Menu panel -->
		<div
			class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-xl dark:bg-gray-800"
			transition:slide={{ duration: 300 }}
			use:clickOutside={closeMobileMenu}
		>
			<!-- Header -->
			<div
				class="flex h-16 items-center justify-between border-b border-gray-200 px-6 dark:border-gray-700"
			>
					...
					>
						<svg
							class={classNames(
								...
								isActive(item.href) ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'
							)}
							fill="none"
							stroke="currentColor"
							stroke-width="2"
			...
			<!-- User Info -->
			<div
				class="absolute right-0 bottom-0 left-0 border-t border-gray-200 p-4 dark:border-gray-700"
			>
