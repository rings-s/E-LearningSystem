...
function createCourseStore() {
    ...
    return {
        ...
        async submitQuiz(quizId) {
            ...
            const responses = Object.entries(state.learningSession.quizAnswers)
        ...
        addNote(note) {
            update(state => ({
                ...
                learningSession: {
                    ...state.learningSession,
                    notes: [...state.learningSession.notes, {
                        id: Date.now(),
                        ...note,
        ...
        deleteNote(noteId) {
            update(state => ({
                ...
                learningSession: {
                    ...state.learningSession,
                    notes: state.learningSession.notes.filter(note => 
                        note.id !== noteId
                    )
                }
            }));
...
export const activeEnrollments = derived(
    courseStore, 
    $store => $store.enrollments.filter(e => e.status === 'in_progress')
);

export const completedEnrollments = derived(
    courseStore,
    $store => $store.enrollments.filter(e => e.status === 'completed')
);

// Learning progress
export const currentLearningProgress = derived(
    courseStore,
    $store => {
